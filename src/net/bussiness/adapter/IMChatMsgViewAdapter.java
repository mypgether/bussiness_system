...
public class IMChatMsgViewAdapter extends BaseAdapter {
	private List<ChatmsgDto> mChatMsgList;
	private EmotionParser mEmotionParser;
	private Context mContext;
	private ChatDetailActivity activity = null;
	...
	public IMChatMsgViewAdapter(Context context, List<ChatmsgDto> list,
			EmotionParser mEmotionParser) {
		this.mContext = context;
		this.mChatMsgList = list;
		this.mEmotionParser = mEmotionParser;
		this.mInflater = LayoutInflater.from(context);
		activity = (ChatDetailActivity) context;
	...
	public int getItemViewType(int position) {
		ChatmsgDto mChatMsg = mChatMsgList.get(position);
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		final ChatmsgDto mChatMsg = mChatMsgList.get(position);
		...
		if (convertView == null) {
			if (isComMsg) {
				...
				if (isFirst) {
					activity.updateChatUser(sender.getPhotoPath());
					isFirst = false;
				}
			} else {
				convertView = mInflater.inflate(
		...
		if (mChatMsg.getMsgType() == 1) {
			...
			application.mAbImageLoader.display(viewHolder.chatImage,
					ConstServer.IM_PERSON_DOWNLOADPHOTO(sender.getUserId(),
							mChatMsg.getUserByReceiverId().getUserId(),
			...
			viewHolder.chatImage.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					application.mAbImageLoader.display(iv, ConstServer
							.IM_PERSON_DOWNLOADPHOTO(sender.getUserId(),
									mChatMsg.getUserByReceiverId().getUserId(),
									new String(mChatMsg.getMsgContent())));
			...
			viewHolder.chatRecord.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					net.post(ConstServer.IM_PERSON_DOWNLOADRECORD(mChatMsg
							.getUserBySenderId().getUserId(), mChatMsg
							.getUserByReceiverId().getUserId(), new String(
							mChatMsg.getMsgContent())), params,
							new AbFileHttpResponseListener() {
								@Override
